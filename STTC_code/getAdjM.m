function [adjM,activeElectrode]  = getAdjM(spikes, method, downSample, lag, time)
...
% if issparse(spikes) % 判断spikes是否为稀疏矩阵
%     spikes = full(spikes); 
% end 
...
%     fs = 25000;
    numChannel = size(spikes,1);
    dtv = lag;
    Time = [0, time]; 
    combChannel = nchoosek(1:numChannel, 2); 
    adjM = NaN(numChannel, numChannel); 
    for channelPairNum = 1:size(combChannel, 1)
        electrode_1 = spikes{combChannel(channelPairNum, 1),1}; 
        electrode_2 = spikes{combChannel(channelPairNum, 2),1};
        N1v = length(electrode_1); 
        N2v = length(electrode_2); 
        spike_times_1 = electrode_1; 
        spike_times_2 = electrode_2;
    ...
    Idx = ~cellfun(@isempty,spikes);
    activeElectrode = find(Idx); 
