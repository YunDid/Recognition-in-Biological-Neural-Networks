...
parentFolder = 'K:\ZY\Voice_Data_Eight\20250314\';
...
for i = 1:length(contents)
    if contents(i).isdir && ~strcmp(contents(i).name, '.') && ~strcmp(contents(i).name, '..')
        ...
        for k = 0:1:20
            ...
            % 加载数据文件
            d = load(fullfile(subFolder, fileNamess{1}));
            d2 = load(fullfile(subFolder, fileNamess{2}));
            d3 = load(fullfile(subFolder, fileNamess{3}));
            d4 = load(fullfile(subFolder, fileNamess{4}));
        ...
        % 保存准确率矩阵到Excel文件
        savepath2 = fullfile(resultFolder, [contents(i).name, '-accuracy.xlsx']);
