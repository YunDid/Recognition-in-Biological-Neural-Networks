function [active_ch] = active_channel(spike_data,spike_data2,spike_data3,t,up,down)
...
for k=2:2:length(Namess2)-2     %Attention
...
sti_start3=extractfield(spike_data3,Namess3{end,1});
sti_end3=extractfield(spike_data3,Namess3{end-1,1});
interval3=[sti_end3+t;sti_start3(2:length(sti_start3)),sti_end3(length(sti_end3))+0.25];
for m=6:6:length(interval3)
    interval3(2,m)=interval3(1,m)+0.24;
end
evoked_data3=zeros(length(sti_start3),64);
for k=2:2:length(Namess3)-2     %Attention
...
b=sum(evoked_data)+sum(evoked_data2)+sum(evoked_data3);
