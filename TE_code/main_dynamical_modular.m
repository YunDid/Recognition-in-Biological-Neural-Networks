%% Date:2024/03/28 动力学丰富度-modular
clc;
clear;
close all
% addpath('/Users/mengweiwei/Desktop/CODEs/BCT/2019_03_03_BCT/')
parentFolder = '/Users/mengweiwei/Desktop/结果/spon/modular';
...
for i = 1:length(contents)
    if contents(i).isdir && ~strcmp(contents(i).name, '.') && ~strcmp(contents(i).name, '..')
        ...
        % 获取子文件夹内的文件列表
        subContents=dir(fullfile(subFolder,'/connection/','*.mat'));
        fileNamess={subContents.name};
        subContents2=dir(fullfile(subFolder,'/sttc/','*.mat'));
        ...
        for j = 1:length(fileNamess)
            path=fullfile(subFolder,'connection',fileNamess(j));
            data=load(path{1,1});
            W=data.nom_connect;
            ...
            path2=fullfile(subFolder,'sttc',fileNamess2(j));
