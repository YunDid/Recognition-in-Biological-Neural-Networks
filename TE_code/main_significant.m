...
for i = 1:length(contents)
    if contents(i).isdir && ~strcmp(contents(i).name, '.') && ~strcmp(contents(i).name, '..')
        ...
        % 获取子文件夹内的文件列表
        subContents=dir(fullfile(subFolder,'te/','*.mat'));
        fileNamess={subContents.name};
        % 构建保存文件夹的完整路径
        savedir1=strcat(subFolder,'/te-network/');
        mkdir(savedir1);
        %         savedir2=strcat(savedir1,'pic/');
        %         mkdir(savedir2)
        % 遍历子文件夹内的每个文件
        for j = 1:length(fileNamess)
            path=fullfile(subFolder,'te',fileNamess(j));
            data=load(path{1,1});
            ...
            savepath=strcat(savedir1,'connect-',fileNamess(j));
