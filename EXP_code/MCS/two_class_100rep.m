...
%% 实验参数
test_batches = 4;
test_reps_per_sequence_per_batch = 25;
...
%% 加药状态下单轮 test：每类 100 次，总计 200 trials
for batch = 1:test_batches
    ClickOnce(record_pos(1), record_pos(2)); % 开始记录
    pause(record_gap)
    ClickOnce(stim_test_pos(1), stim_test_pos(2)); % 选择测试刺激器

    for rep = 1:test_reps_per_sequence_per_batch
        sequence_order = randperm(2);

        for p = 1:2
            sequence_id = sequence_order(p);
...
protocol.test_reps_per_sequence_per_batch = test_reps_per_sequence_per_batch;
protocol.test_reps_per_sequence_total = test_batches * test_reps_per_sequence_per_batch;
protocol.test_trials_total = 2 * protocol.test_reps_per_sequence_total;
protocol.stim_step_interval = stim_step_interval;
protocol.trial_interval = trial_interval;

save('stim_labels_two_class_100rep_drug_test.mat', ...
    'protocol', 'stim_labels_test', 'stim_source_ids_test', ...
    'stim_batch_ids_test', 'stim_trial_ids_test');
disp('Two-class drug-condition test labels saved to stim_labels_two_class_100rep_drug_test.mat.');
